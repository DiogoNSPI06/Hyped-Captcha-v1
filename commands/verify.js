...
module.exports.run = async (client, message, args, prefix, color, config) => {
  message.delete()
  let isGuild = db.get(`isGuild_${message.guild.id}`);
  let beforerole = db.get(`role_${message.guild.id}`);
  ...
  if(!isGuild || isGuild === false) {
    return message.channel.send(`:x: | This guild is not active! Use \`${prefix}setconfig\`!`)
  }
  ...
  Jimp.read(`https://imgur.com/yF4Mbkf.png`, function (err, image) {
    ...
    Jimp.loadFont(Jimp.FONT_SANS_64_BLACK).then(function (font) {
      ...
      image.getBuffer(Jimp.MIME_PNG, (err, buffer) => {
        ...
        message.author.send(`${message.author}, Write what is in this image!`, attachment)
      })
    })
  })
  ...
  main.on('collect', a => {
    if(a.content === captcha) {
      message.author.send(":white_check_mark: | Captcha complete!")
      message.member.roles.add(role)
    }

    if(a.content !== captcha) return message.author.send(":x: | Wrong anwser, redo the captcha pls.")
  })
}